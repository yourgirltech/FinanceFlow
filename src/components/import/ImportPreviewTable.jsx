...
export default function ImportPreviewTable({ rows, region, onToggleRow, onChangeCategory }) {
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-white/[0.04] border border-line dark:border-white/10 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-line dark:border-white/10 bg-surface/50 dark:bg-white/[0.02]">
      ...
      <div className="max-h-[420px] overflow-y-auto">
        {rows.map((row) => (
          <div
            key={row.id}
            className={`grid grid-cols-[28px_90px_1fr_150px_110px] items-center gap-3 px-5 py-2.5 border-b border-line dark:border-white/10 last:border-0 transition-opacity ${
          ...
          >
            ...
            <select
              ...
              className="h-8 rounded-lg border border-line dark:border-white/15 bg-white dark:bg-white/[0.04] px-2 text-[12px] text-navy dark:text-white focus:outline-none focus:border-gold [color-scheme:light] dark:[color-scheme:dark]"
            >
              {CATEGORIES.map((c) => (
