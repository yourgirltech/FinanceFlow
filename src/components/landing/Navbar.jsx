  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled || menuOpen
          ? 'bg-white/95 dark:bg-navy/95 backdrop-blur-md border-b border-line dark:border-white/10 shadow-[0_1px_16px_rgba(11,18,32,0.06)]'
          : 'bg-transparent'
      }`}
    >
      ...
      >
        <div className="px-5 pb-7 pt-3 border-t border-line dark:border-white/10 bg-white dark:bg-navy">
          <div className="flex flex-col gap-1 mb-5">
            {links.map((link) => (
              <a
                ...
                className="flex items-center gap-2 py-3 text-[15px] font-medium text-navy dark:text-white/80"
              >
                {link.label}
                {link.badge && (
                  <span className="text-[9px] font-semibold uppercase tracking-wide text-gold bg-gold-soft dark:bg-gold/15 rounded-full px-1.5 py-0.5">
          ...
          <div className="flex items-center justify-between py-4 border-t border-line dark:border-white/10">
            <span className="text-xs text-slate-light dark:text-white/35">Region &amp; currency</span>
            <RegionSwitcher />
          </div>

          <div className="flex items-center justify-between py-4 border-t border-line dark:border-white/10">
            <span className="text-xs text-slate-light dark:text-white/35">Appearance</span>
            <ThemeToggle />
          </div>
