  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-medium text-slate dark:text-white/50">{label}</span>
        {icon && (
          <div className="h-8 w-8 rounded-lg bg-surface dark:bg-white/[0.06] flex items-center justify-center">
            {icon}
          </div>
        )}
      </div>
      <p className={`font-tabular text-2xl font-semibold tracking-tight ${toneClasses}`}>
        {isMoney ? formatMoney(animated, region) : `${Math.round(animated)}%`}
      </p>
      {delta && (
        <p className={`text-xs mt-1.5 font-medium ${delta.up ? 'text-emerald' : 'text-red'}`}>
          {delta.up ? '↑' : '↓'} {delta.text}
        </p>
      )}
    </div>
